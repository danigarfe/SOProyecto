...
namespace PongSO
{
    public partial class Form2 : Form
    {
        ...
        int puertoservidor = 50068; //poner aqui el puerto del servidor
        public Form2()
        {
            InitializeComponent();
        }
        ...
        {
            bool simbolomalo = false; //PARA EVITAR ERRORES EN EL REGISTRO/INICIO DE SESIÓN
            for(int i = 0; i < nombre.Text.Length && simbolomalo==false; i++)
            {
                if (nombre.Text[i] == '=' || nombre.Text[i]=='/' || nombre.Text[i] == ' ')
                    simbolomalo = true;
            }
            for (int i = 0; i < contraseña.Text.Length && simbolomalo == false; i++)
            {
                if (contraseña.Text[i] == '/' || contraseña.Text[i] == ' ')
                    simbolomalo = true;
            }
            if (nombre.Text.Length>0 && contraseña.Text.Length > 0 && simbolomalo==false)
            {
            ...
            {
                MessageBox.Show("Usuario y/o contraseña erróneos.");
            }
        }
